...
if module_option == None or selected_machine == None:
    ...
    # Perform Dynamic Filtering on the date file
    custom_rainbow_divider()
    st.markdown(":green[ **Select the required date to display Tabulated Log Data for {} under {}**]".format(module_option, selected_machine))

    d = st.date_input("Select the required date", datetime.date.today())
    date_option = d.strftime("%m-%d")
    print(date_option)
    ...
    try:
        module_alarm_summary = open(selected_machine + '_' + module_option + '_' + date_option + '_' + 'combined_file.csv', "wb")
        res4_attributes, res4size = conn.retrieveFile(shareName,
                                                      os.path.join(folderName, selected_machine, module_option + '/' +
                                                                   date_option + '.csv'),
                                                      module_alarm_summary)
        print(module_option)
        print(module_alarm_summary)
        print("success in file generate")
        time.sleep(3)

        df_log_module = pd.read_csv(selected_machine + '_' + module_option + '_' + date_option + '_'  + 'combined_file.csv',
                                        encoding='unicode_escape')
        print(df_log_module.head())

        if 'Unnamed: 0' in df_log_module.columns:
            df_log_module = df_log_module.drop(columns=['Unnamed: 0'])

        # Rename the dataframe columns
        df_log_module.rename(columns={'0': 'Date', '1': 'Timestamp', '2': 'log_entry'}, inplace=True)
        print(df_log_module.columns)
        ...
        st.info(":red[Unhandled error, please inform Admin]")
